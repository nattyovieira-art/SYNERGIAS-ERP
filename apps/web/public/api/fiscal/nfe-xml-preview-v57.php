...
const NFE_XML_PREVIEW_VERSION = 'V57';
...
function cortarTexto(string $v, int $max): string {
    return function_exists('mb_substr')
        ? mb_substr($v, 0, $max, 'UTF-8')
        : substr($v, 0, $max);
}
function textoMaiusculo(string $v): string {
    return function_exists('mb_strtoupper')
        ? mb_strtoupper($v, 'UTF-8')
        : strtoupper($v);
}
function normalizarTexto(string $v, int $max): string {
    $v = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/u', '', $v) ?? '';
    return cortarTexto(trim($v), $max);
}
...
try {
    ...
    $cidadeDest = textoMaiusculo(texto($venda['faturamentoCidade'] ?? ''));
    $cidadeEmit = textoMaiusculo(texto($fiscal['municipio'] ?? ''));
    ...
    $transp=$doc->createElement('transp'); $inf->appendChild($transp); add($doc,$transp,'modFrete','9');
    ...
    $codigo = substr(hash('sha256', get_class($e).'|'.$e->getMessage()), 0, 10);
    error_log('[Synergias NF-e XML V57]['.$codigo.'] '.get_class($e).': '.$e->getMessage());
    responder(500,[
        'ok'=>false,
        'mensagem'=>'Falha interna ao montar o XML: '.$e->getMessage(),
        'codigoErro'=>$codigo,
        'versao'=>NFE_XML_PREVIEW_VERSION
    ]);
}
