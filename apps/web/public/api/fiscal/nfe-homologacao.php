...
const NFE_HOMOLOG_VERSION='V52';
const NFE_NS='http://www.portalfiscal.inf.br/nfe';
const DS_NS='http://www.w3.org/2000/09/xmldsig#';
const AUT_URL='https://nfe-homologacao.sefazrs.rs.gov.br/ws/NfeAutorizacao/NFeAutorizacao4.asmx';
...
function transmitir(string $xml,array $m):string{$cert=$m['cert'];foreach($m['extras'] as $x)if(is_string($x)&&trim($x)!=='')$cert.="\n".trim($x)."\n";$cf=tempPem('cert',$cert);$kf=tempPem('key',$m['key']);function sxV72GarantirDescricaoPagamento99(string $xml): string {
    $doc = new DOMDocument('1.0', 'UTF-8');
    $doc->preserveWhiteSpace = false;
    $doc->formatOutput = false;

    if (!@$doc->loadXML($xml, LIBXML_NOBLANKS | LIBXML_NONET)) {
        throw new RuntimeException('XML invÃ¡lido ao normalizar a forma de pagamento.');
    }

    $xpath = new DOMXPath($doc);
    $detalhes = $xpath->query('//*[local-name()="detPag"]');

    if ($detalhes !== false) {
        foreach ($detalhes as $detPag) {
            if (!$detPag instanceof DOMElement) {
                continue;
            }

            $tPag = $xpath->query('./*[local-name()="tPag"]', $detPag)->item(0);
            $xPag = $xpath->query('./*[local-name()="xPag"]', $detPag)->item(0);

            if (trim((string)($tPag?->textContent ?? '')) !== '99' || $xPag instanceof DOMElement) {
                continue;
            }

            $descricao = $doc->createElementNS(
                'http://www.portalfiscal.inf.br/nfe',
                'xPag',
                'OUTROS - CONFORME PEDIDO'
            );

            $vPag = $xpath->query('./*[local-name()="vPag"]', $detPag)->item(0);

            if ($vPag instanceof DOMNode) {
                $detPag->insertBefore($descricao, $vPag);
            } else {
                $detPag->appendChild($descricao);
            }
        }
    }

    $resultado = $doc->saveXML($doc->documentElement);

    if (!is_string($resultado) || $resultado === '') {
        throw new RuntimeException('Falha ao normalizar a descriÃ§Ã£o do pagamento.');
    }
...
$docHomologacao=new DOMDocument('1.0','UTF-8');
$docHomologacao->preserveWhiteSpace=false;
$docHomologacao->formatOutput=false;
if(!@$docHomologacao->loadXML($xml,LIBXML_NOBLANKS|LIBXML_NONET))throw new RuntimeException('XML inválido antes da assinatura.');
$xpHomologacao=new DOMXPath($docHomologacao);
$nomeDestHomologacao=$xpHomologacao->query('//*[local-name()="dest"]/*[local-name()="xNome"]')->item(0);
if(!$nomeDestHomologacao instanceof DOMElement)throw new RuntimeException('Razão social do destinatário não encontrada no XML.');
$nomeDestHomologacao->nodeValue='NF-E EMITIDA EM AMBIENTE DE HOMOLOGACAO - SEM VALOR FISCAL';
$xmlHomologacao=$docHomologacao->saveXML($docHomologacao->documentElement);
if(!is_string($xmlHomologacao)||$xmlHomologacao==='')throw new RuntimeException('Falha ao preparar XML de homologação.');
$xml=$xmlHomologacao;$xml=sxV72GarantirDescricaoPagamento99($xml);$m=materialA1();$ass=assinar($xml,$m);$ret=parseRet(transmitir($ass['xml'],$m),$ass['xml'],$ass['chave']);out(200,['ok'=>true,'versao'=>NFE_HOMOLOG_VERSION,'ambiente'=>'HOMOLOGACAO','assinado'=>true,'autorizada'=>$ret['autorizada'],'cStat'=>$ret['cStat'],'motivo'=>$ret['motivo'],'protocolo'=>$ret['protocolo'],'recebidoEm'=>$ret['recebidoEm'],'chaveAcesso'=>$ass['chave'],'xmlAssinadoBase64'=>base64_encode($ass['xml']),'xmlProcessadoBase64'=>$ret['xmlProcessado']!==''?base64_encode($ret['xmlProcessado']):'','danfeUrl'=>$ret['autorizada']?'/api/fiscal/nfe-danfe.php?chave='.$ass['chave']:'']);}catch(Throwable $e){error_log('[NFE V52] '.$e->getMessage());out(500,['ok'=>false,'mensagem'=>$e->getMessage()]);}
