...
const NFE_XML_PREVIEW_VERSION = 'V62';
...
try {
    ...
    $ambiente = strtoupper(sxV62Texto($body['ambiente'] ?? 'HOMOLOGACAO'));
    ...
    $nNF = max(2384, (int)($num['sxV62Numero'] ?? 2384));
    ...
    $cMunDest = sxV62Digitos($venda['faturamentoCodigoIbge'] ?? '');
    ...
    if ($erros) sxV62Responder(200, ['ok'=>true,'pronto'=>false,'ambiente'=>'HOMOLOGACAO','versao'=>NFE_XML_PREVIEW_VERSION,'chaveAcesso'=>'','sxV62Numero'=>(string)$nNF,'serie'=>(string)$serie,'xml'=>'','xmlBase64'=>'','erros'=>array_values(array_unique($erros)),'avisos'=>$avisos,'geradoEm'=>gmdate('c')]);
    ...
    foreach($itens as $i=>$item){
        ...
        $vProd=round($q*$vUn,2); $det=$doc->createElement('det'); $det->setAttribute('nItem',(string)($i+1)); $inf->appendChild($det); $prod=$doc->createElement('prod'); $det->appendChild($prod); sxV62Add($doc,$prod,'cProd',sxV62NormalizarTexto(sxV62Texto($item['codigoProduto'] ?? ($i+1)),60)); $cEAN=sxV62Digitos($item['codigoBarras'] ?? ''); sxV62Add($doc,$prod,'cEAN',in_array(strlen($cEAN),[8,12,13,14],true)?$cEAN:'SEM GTIN'); sxV62Add($doc,$prod,'xProd',sxV62NormalizarTexto(sxV62Texto($item['descricao']),120)); sxV62Add($doc,$prod,'NCM',sxV62Digitos($item['ncm'])); if(strlen(sxV62Digitos($item['cest'] ?? ''))===7) sxV62Add($doc,$prod,'CEST',sxV62Digitos($item['cest'])); sxV62Add($doc,$prod,'CFOP',sxV62Digitos($item['cfop'])); $un=sxV62NormalizarTexto(sxV62Texto($item['unidade'] ?? 'UN'),6); sxV62Add($doc,$prod,'uCom',$un); sxV62Add($doc,$prod,'qCom',sxV62FormatarQuantidade($q)); sxV62Add($doc,$prod,'vUnCom',number_format($vUn,10,'.','')); sxV62Add($doc,$prod,'vProd',sxV62Moeda($vProd)); sxV62Add($doc,$prod,'cEANTrib',in_array(strlen($cEAN),[8,12,13,14],true)?$cEAN:'SEM GTIN'); sxV62Add($doc,$prod,'uTrib',sxV62NormalizarTexto(sxV62Texto($item['unidadeTributavel'] ?? $un),6)); sxV62Add($doc,$prod,'qTrib',sxV62FormatarQuantidade($q)); sxV62Add($doc,$prod,'vUnTrib',number_format($vUn,10,'.','')); sxV62Add($doc,$prod,'indTot','1');
    ...
    $transp=$doc->createElement('transp'); $inf->appendChild($transp); sxV62Add($doc,$transp,'modFrete','9');
    $pag=$doc->createElement('pag'); $inf->appendChild($pag); $detPag=$doc->createElement('detPag'); $pag->appendChild($detPag); sxV62Add($doc,$detPag,'indPag','1'); sxV62Add($doc,$detPag,'tPag','99'); sxV62Add($doc,$detPag,'vPag',sxV62Moeda($vNF));
    ...
    sxV62Responder(200,['ok'=>true,'pronto'=>true,'ambiente'=>'HOMOLOGACAO','versao'=>NFE_XML_PREVIEW_VERSION,'chaveAcesso'=>$chave,'sxV62Numero'=>(string)$nNF,'serie'=>(string)$serie,'xml'=>$xml,'xmlBase64'=>base64_encode($xml),'erros'=>[],'avisos'=>$avisos,'geradoEm'=>gmdate('c')]);
} catch(Throwable $e){
