...
function caminhoConfigAuth(): string { $local=__DIR__.'/auth-config.php'; if(is_file($local)) return $local; $cands=[rtrim((string)(getenv('HOME')?:''),'/\\').'/synergias_private/auth/config.php',dirname(__DIR__,2).'/synergias_private/auth/config.php',dirname((string)($_SERVER['DOCUMENT_ROOT']??__DIR__)).'/synergias_private/auth/config.php',dirname(__DIR__,3).'/synergias_private/auth/config.php']; foreach(array_unique($cands) as $p){$r=realpath($p); if($r!==false&&is_file($r)) return $r;} throw new RuntimeException('Configuração privada de autenticação não encontrada.'); }
...
function exigirAutenticacao(): array { iniciarSessaoSegura(); $u=usuarioAutenticado(); if($u===null)responder(401,['ok'=>false,'error'=>'Sessão não autenticada.']); return $u; }
...
set_exception_handler(function(Throwable $e): void {error_log('[Synergias ERP API] '.$e->getMessage());responder(500,['ok'=>false,'error'=>$e->getMessage()]);});
