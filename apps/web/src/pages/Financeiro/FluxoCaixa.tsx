  ...
  return grupos
    .map((nomeGrupo) => {
      ...
      const categorias = Array.from(new Set(linhasGrupo.map((linha) => linha.categoria)))
        .map((nomeCategoria) => {
          const linhasCategoria = linhasGrupo.filter(
            (linha) => linha.categoria === nomeCategoria,
          )
...
function FluxoCaixa() {
    ...
    contasReceber.forEach((conta) => {
      ...
      if (realizado) {
        linhas.push({
          ...
          categoria: conta.formaPagamento || conta.tipoCobranca || 'Recebimentos de vendas',
          descricao: conta.descricao || 'Conta recebida',
          ...
          valor: valorRecebidoConta(conta),
        })
      }
      ...
      if (saldoAberto > 0 && conta.status !== 'Paga') {
        linhas.push({
          ...
          categoria: conta.formaPagamento || conta.tipoCobranca || 'Recebimentos previstos',
  ...
  return (
    <main className="financeiro-page">
      ...
      <section className="financeiro-content fluxo-caixa-page">
        <PageHeader
          category="Financeiro"
          title="Fluxo de Caixa"
          subtitle="Visão analítica e projetada das entradas e saídas ligadas às contas a receber e a pagar."
        />

        <div className="financeiro-toolbar financeiro-toolbar-padrao">
          <div className="financeiro-toolbar-left-actions">
            <button className="financeiro-icon-button financeiro-icon-back" onClick={() => navigate('/financeiro')} title="Voltar" aria-label="Voltar">
              <ArrowLeft size={25} strokeWidth={2.4} />
            </button>
            <button className="financeiro-icon-button financeiro-icon-list" title="Lista" aria-label="Lista" onClick={() => setVisao('movimentos')}>
              <List size={25} strokeWidth={2.4} />
            </button>
            <button className="financeiro-icon-button financeiro-icon-search" title="Buscar" aria-label="Buscar" onClick={() => document.querySelector<HTMLInputElement>('.fluxo-caixa-busca input')?.focus()}>
              <Search size={25} strokeWidth={2.4} />
            </button>
            <div className="financeiro-busca fluxo-caixa-busca">
              <Search size={18} />
              <input
                value={busca}
            ...
            <button className={`financeiro-filter-button ${filtrosAtivos ? 'ativo' : ''}`} onClick={() => setMostrarFiltros((atual) => !atual)} title="Filtros" aria-label="Filtros">
              <Filter size={20} />
          ...
          <div className="financeiro-toolbar-right-actions">
            <button className="financeiro-icon-button financeiro-icon-print" onClick={() => window.print()} title="Imprimir" aria-label="Imprimir">
              <Printer size={25} strokeWidth={2.4} />
            </button>
            <button className="financeiro-icon-button financeiro-icon-refresh" onClick={atualizarDados} title="Atualizar" aria-label="Atualizar">
        ...
        {mostrarFiltros && (
          <section className="fluxo-caixa-filtros">
            ...
            <label>
              <span>Contas</span>
              <select value={contaFiltro} onChange={(event) => setContaFiltro(event.target.value)}>
                <option>Todas</option>
                {dados.contasDisponiveis.map((conta) => <option key={conta}>{conta}</option>)}
              </select>
            </label>
            <label>
              <span>Situação</span>
              <select value={situacaoFiltro} onChange={(event) => setSituacaoFiltro(event.target.value as 'Todas' | SituacaoLinha)}>
                <option>Todas</option>
            ...
            <label>
              <span>Movimento</span>
              <select value={tipoFiltro} onChange={(event) => setTipoFiltro(event.target.value as 'Todos' | 'Entradas' | 'Saídas')}>
                <option>Todos</option>
            ...
            <label>
              ...
              <input value={saldoInicial} onChange={(event) => setSaldoInicial(event.target.value)} placeholder="0,00" />
            </label>
            <div className="fluxo-caixa-filtros-acoes">
        ...
        <section className="fluxo-caixa-resumo">
          <article>
            ...
            <small>Valor informado para o período</small>
          </article>
          <article>
            <span>Entradas</span>
            <strong>{moeda(dados.totalEntradas)}</strong>
            <small>{moeda(dados.entradasRealizadas)} realizado · {moeda(dados.entradasPrevistas)} previsto</small>
          </article>
          <article>
            <span>Saídas</span>
            <strong>{moeda(dados.totalSaidas)}</strong>
            <small>{moeda(dados.saidasRealizadas)} realizado · {moeda(dados.saidasPrevistas)} previsto</small>
          </article>
          <article className="saldo-projetado">
            <span>Saldo final projetado</span>
            <strong>{moeda(dados.saldoProjetado)}</strong>
        ...
        {visao === 'analitica' ? (
          <section className="fluxo-caixa-analitico">
            <div className="fluxo-caixa-linha-principal saldo-inicial">
              <span>Saldo inicial</span>
              <strong>{moeda(dados.saldo)}</strong>
            </div>

            <button className="fluxo-caixa-linha-principal entrada" onClick={() => alternarAberto('entradas')}>
              <span>{abertos.entradas ? <ChevronDown size={20} /> : <ChevronRight size={20} />} Receitas / Entradas</span>
              <strong>{moeda(dados.totalEntradas)}</strong>
            </button>
            ...
            {abertos.entradas && (
              <div className="fluxo-caixa-grupos">
                {dados.gruposEntradas.map((grupo) => {
                  ...
                  return (
                    <div className="fluxo-caixa-grupo" key={chaveGrupo}>
                      <button className="fluxo-caixa-linha-grupo" onClick={() => alternarAberto(chaveGrupo)}>
                        <span>{abertos[chaveGrupo] ? <ChevronDown size={18} /> : <ChevronRight size={18} />} {grupo.nome}</span>
                      ...
                      {abertos[chaveGrupo] && grupo.categorias.map((categoria) => {
                        ...
                        return (
                          <div key={chaveCategoria} className="fluxo-caixa-categoria">
                            <button className="fluxo-caixa-linha-categoria" onClick={() => alternarAberto(chaveCategoria)}>
                              <span>{abertos[chaveCategoria] ? <ChevronDown size={16} /> : <ChevronRight size={16} />} {categoria.nome}</span>
                              <strong>{moeda(categoria.total)}</strong>
                            </button>
                            {abertos[chaveCategoria] && (
                              <div className="fluxo-caixa-itens">
                                {categoria.linhas.map((linha) => (
                                  <div className="fluxo-caixa-item" key={linha.id}>
                                    <div><strong>{formatarData(linha.data)}</strong><span>{linha.descricao}</span><small>{linha.origem} · {linha.conta}</small></div>
                                    <div><span className={`fluxo-situacao ${linha.situacao.toLowerCase()}`}>{linha.situacao}</span><strong>{moeda(linha.valor)}</strong></div>
                                  </div>
                                ))}
                              </div>
                            )}
            ...
            <button className="fluxo-caixa-linha-principal saida" onClick={() => alternarAberto('saidas')}>
              <span>{abertos.saidas ? <ChevronDown size={20} /> : <ChevronRight size={20} />} Despesas / Saídas</span>
              <strong>{moeda(dados.totalSaidas)}</strong>
            </button>
            ...
            <div className="fluxo-caixa-linha-principal saldo-final">
              <span>Saldo final projetado</span>
              <strong>{moeda(dados.saldoProjetado)}</strong>
            </div>
          </section>
          ...
          <section className="fluxo-caixa-tabela-card">
            <div className="novo-table-wrap">
              <table className="novo-table fluxo-caixa-tabela">
                ...
                <tbody>
                  {dados.movimentos.map((linha) => (
                    <tr key={linha.id}>
                      <td>{formatarData(linha.data)}</td>
                      <td><span className={`fluxo-situacao ${linha.situacao.toLowerCase()}`}>{linha.situacao}</span></td>
                      <td><span className={`fluxo-tipo ${linha.tipo}`}>{linha.tipo === 'entrada' ? 'Entrada' : 'Saída'}</span></td>
                      <td>{linha.categoria}</td>
                      <td><strong>{linha.descricao}</strong><small>{linha.origem}</small></td>
                      <td>{linha.conta}</td>
                      <td className={linha.tipo === 'entrada' ? 'valor-entrada' : 'valor-saida'}>{linha.tipo === 'entrada' ? '+' : '−'} {moeda(linha.valor)}</td>
                      <td><strong>{moeda(linha.acumulado)}</strong></td>
                    </tr>
                  ))}
